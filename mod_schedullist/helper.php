 ...
 * @since  1.6
 */
abstract class ModSchedullistHelper
{
	/**
	...
	public function getAjax()
    {
       //return "hi ajax";
        ...
        $tagids = $app->input->getString('tagids','');
        $catid = $app->input->get('catid', '');

        $params1 = new JRegistry();
        //error_log(var_export($app->input['data'],true));
        //error_log($tagids);
        $params1->set('filter_tag', explode(',', $tagids));
        $params1->set('catid', $catid);

        $articles = ModSchedullistHelper::getList($params1);
		//error_log(__LINE__);
		//error_log(var_export($articles,true));
        //error_log(var_export($params1,true));
		...
		foreach ($articles as $item){
			$flagAllTags = true;
			//error_log(var_export($item->tags,true));
			if (true) {
		    	...
		    	foreach ($arr as $it) {
		    		error_log(__LINE__);
		    		$idmap[$it]=0;//init the count of tag
		    	}
		    	//error_log(var_export($item,true));
		    	error_log(var_export($item->tags->itemTags,true));
		    	foreach ($item->tags->itemTags as $key => $value) {

		    		if (isset($idmap[$value->id]) && $idmap[$value->id]>=0 ){
		    			$idmap[$value->id]=$idmap[$value->id]+1;
		    			error_log(__LINE__);
		    			error_log($value->id);
		    			error_log($idmap[$value->id]);
		    		}
		    		
		    	}
		    	error_log(var_export($idmap,true));
		    	...
		    	error_log(var_export($flagAllTags,true));
			}
			if($flagAllTags) {
				error_log(__LINE__);
				$retVal = $retVal.'<hr class="uk-grid-divider"><div class="row">';
				...
				foreach ($fields as $field){
					...
					if (isset($field->value) && $field->value != '') {
						$retVal = $retVal.'<div class="col-sm">'.$field->value.'</div>';
					}
				}
				$retVal = $retVal.'</div><hr class="uk-grid-divider">';
			}
		}

       return $retVal;//getHtmlList($articles);
    }

    function isContainAllTags($tags,$tagids)
    {
    }

    public function getHtmlList($articles){
    	$retVal = '';

		foreach ($articles as $item){
			$retVal = $retVal.'<div class="row">';
